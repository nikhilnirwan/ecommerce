...
const CheckUsed = async (userid, coupon) => {
    ...
    const count = await Redeem.find({ user_id: userid, coupon_code: coupon, coupon_type: "order" });
...
router.post("/validate-coupon", upload.none(), function (req, res, next) {
    // all offer
    ...
    if (err.error) {
        ...
        Coupons.findOne({ coupon_code: req.body.coupon_code, status:{$ne: false}, coupon_type: "order"})
